...
import { log, xpMoney } from './progress.js';
import { S, costN, has, lvReq, maxN, unl } from './state.js';
...
export function buyCrew(c){
  ...
  S.money -= cost; S.owned[c.id] = (S.owned[c.id]||0) + n; xpMoney(cost, .05);
  sfx('buy'); emit('bought', c.id, n);
  syncLamps(); updateUI();
}
export function buyUp(u){
  if (has(u.id) || S.money < u.cost) return;
  S.money -= u.cost; S.ups[u.id] = true; xpMoney(u.cost, .05); sfx('buy');
