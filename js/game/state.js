...
import { BIZ, CLICK_UPS, CREW, FIND_SETS, GAL_DEPTH, METALS, MK, REGIMES, SAVE_KEY, SPREAD, STOCKS, STRATA, UNLOCKS } from '../data/content.js';
...
export const fresh = () => ({v:3, money:0, stock:{au:0,ag:0,cu:0}, vein:'au', opened:{au:true}, mined:0, earned:0, allEarned:0, clicks:0, sold:0, best:0, nuggets:0,
  ...
  perks:{}, legSpent:0, autoMine:{on:true, th:0.02}, autoMineT:{}, wind:0, windF:.8, plants:{bio:0, fab:0}, obj:0, tutSeen:{}, finds:{}, lastStratum:0, saved:Date.now()});

export const has = id => !!S.ups[id];
export const sk = id => !!S.skills[id];
export const lvReq = key => UNLOCKS.find(u=>u.key===key).lv;
export const unl = key => S.level >= lvReq(key);
...
const moralF = () => S.moral >= 60 ? 1 : S.moral/60;
...
export const clickEq = () => (0.025*CLICK_UPS.reduce((a,id)=>a*(has(id)?2:1),1)*(sk('m1')?1.5:1)*achMult()*legMult()) + (has('u_detector') ? gpsEq()*0.05 : 0);
...
export const depth = () => 2 + Math.sqrt(S.mined)*6;
...
export const xpNeed = L => Math.round(60*Math.pow(1.6, L-1));
...
export function jewelRate(){ const st = S.biz.joyeria; if (!st || !st.lv || st.paused) return 0; return 0.08*st.lv*Math.pow(1.06,st.lv)*(st.mgr?2:1)*boostOf(st)*bizMult()/(achMult()*legMult()); }
